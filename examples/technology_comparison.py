...
capacitor_requirements = pecst.CapacitorRequirements(
    ...
    capacitor_type_list=[pecst.CapacitorType.FilmCapacitor, pecst.CapacitorType.CeramicCapacitor],
...
for count, c_db in enumerate(c_db_list):
    ax[0].scatter(c_db["volume_total"] * pecst.QUBIC_METER_TO_QUBIC_DECI_METER, c_db["power_loss_total"],
               color=color_list[count], label=c_name_list[count])
    ax[1].scatter(c_db["area_total"] * pecst.SQUARE_METER_TO_SQUARE_CENTI_METER, c_db["power_loss_total"],
               color=color_list[count], label=c_name_list[count])
